...
> 对应模块：APP 应用管理 · CFG 配置中心 · REG 注册中心 · LOG 日志中心 · PLT 平台管理 · DBM 数据中间件 · SVR 基础能力
...
| 控制器 | 路由 | 说明 | 对应模块 |
|--------|------|------|----------|
| `NodeController` | `/node` | 节点注册/心跳/升级/命令 | NODE |
| `AppController` | `/app` | 应用注册/心跳 | APP |
| `ConfigController` | `/config` | 配置拉取/设置 | CFG |
| `TraceController` | `/trace` | 监控数据上报 | APM |
| `LogController` | `/log` | 日志接收 | LOG |
| `DeployController` | `/deploy` | 发布接口 | PUB |
| `GatewayController` | `/gateway` | 网关配置下发 | GW |
| `OAuthController` | `/oauth` | OAuth 令牌服务 | PLT |
| `AgentDeployController` | `/agentdeploy` | Agent 远程部署 | NODE |
| `CubeController` | `/cube` | 魔方数据接口（附件等） | PUB |
| `ApiController` | `/api` | 系统信息/接口列表 | SVR |

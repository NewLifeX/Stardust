...
namespace Stardust.Data.Monitors
{
    ...
    public partial class TraceMinuteStat : Entity<TraceMinuteStat>
    {
        ...
        public static IList<TraceMinuteStat> FindAllByAppIdWithCache(Int32 appId, DateTime date)
        {
            ...
            if (_cache.TryGet<IList<TraceMinuteStat>>(key, out var list) && list != null) return list;
        ...
        {
            var key = $"TraceMinuteStat:FindByTrace:{model.Time}#{model.AppId}#{model.Name}";
        ...
        {
            // 高并发下获取或新增对象
            return GetOrAdd(model, FindByTrace, k => new TraceMinuteStat { StatTime = model.Time, AppId = model.AppId, Name = model.Name });
        }
        #endregion
    }
}
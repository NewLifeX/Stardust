...
public partial class NodeOnline : Entity<NodeOnline>, IOnlineModel2
{
    ...
    private void Fill(PingInfo inf)
    {
        ...
        if (inf.GatewayLatency > 0) online.GatewayLatency = inf.GatewayLatency;
        if (inf.GatewayLossRate > 0) online.GatewayLossRate = inf.GatewayLossRate;
        if (inf.DnsLatency > 0) online.DnsLatency = inf.DnsLatency;
        if (inf.DnsLossRate > 0) online.DnsLossRate = inf.DnsLossRate;
        if (inf.ServerLatency > 0) online.ServerLatency = inf.ServerLatency;
        if (inf.ServerLossRate > 0) online.ServerLossRate = inf.ServerLossRate;
    ...
    private void CreateData(PingInfo inf, String ip)
    {
        ...
        var data = new NodeData
        {
            ...
            GatewayLatency = inf.GatewayLatency,
            GatewayLossRate = inf.GatewayLossRate,
            DnsLatency = inf.DnsLatency,
            DnsLossRate = inf.DnsLossRate,
            ServerLatency = inf.ServerLatency,
            ServerLossRate = inf.ServerLossRate,
            Uptime = inf.Uptime,
            Delay = inf.Delay,
            LocalTime = dt,
            Offset = olt.Offset,
